...
- 原生工作区详情顶部提供 Command Center，把生命周期进度、主路径推荐、范围 -> worktree -> 风险 -> 任务 -> 交付 -> Codex 交接的会话路径、Codex 继续、本地检查结果、Finder 和 Terminal 放到同一个固定入口。
- 原生工作区详情顶部提供状态概览，把生命周期、分支、服务、风险、任务、交付和最近本地检查状态放在进入详情后的第一屏。
...
- 原生工作区详情提供 Workflow 汇总，集中展示开放任务、阻塞任务、交付状态、交付焦点卡、交付前检查、生命周期写回建议、任务文档、交付记录、本地检查和 Codex 交接，并使用中文优先的主动作标签。
...
每个工作区详情顶部会先展示状态概览，汇总生命周期、目标分支、服务/worktree、风险、任务、交付和最近本地检查。随后 `Command Center` 会汇总生命周期进度、分支确认状态、服务和 worktree 情况、风险等级、开放任务，并先给出一个带原因的主路径推荐；随后用会话路径把范围、worktree、风险、任务、交付和 Codex 交接压缩成一组可点击状态，再保留本地检查、Finder 和 Terminal 等工具入口。运行本地检查后，这里会保留一张检查回执，展示状态、风险/交付/任务/worktree 指标、审计写入状态和可复制摘要。

每个工作区详情中都有 `Workflow` 区块，把任务和交付状态放在一起：顶部交付焦点卡会从目标分支、服务范围、worktree、阻塞/开放任务、风险、交付记录、SQL、未提交服务、进入交付和标记完成中选择一个下一步动作；下方继续展示开放任务数、阻塞任务数、交付记录是否需要复核，并汇总目标分支、服务 worktree、任务关闭、风险、SQL、未提交服务和交付记录这些交付前检查项；也可以直接打开 `tasks.md`、`交付记录.md`、运行本地检查或交接 Codex。
