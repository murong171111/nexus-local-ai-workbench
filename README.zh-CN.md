...
创建前会展示目标路径、分支、服务范围摘要，并要求确认本地写入。创建动作会写入标准 Markdown 文档，并把选中的服务记录到 `services.md` 和 `branches.md`。同时会生成 `bootstrap-report.md`、`scripts/worktree-commands.sh`，并写入一条本地审计事件。
...
每个工作区详情顶部都有 `Command Center`。它会汇总生命周期进度、分支确认状态、服务和 worktree 情况、风险等级、开放任务，并把主路径收敛为：继续交给 Codex、打开生命周期下一步、运行本地检查，或交给 Finder/Terminal 手工处理。

每个工作区详情中都有 `Workflow` 区块，把任务和交付状态放在一起：展示开放任务数、阻塞任务数、交付记录是否需要复核，并可直接打开 `tasks.md`、`交付记录.md`、运行本地检查或交接 Codex。
