...
- 原生工作区详情顶部提供 Command Center，把生命周期进度、主路径推荐、范围 -> worktree -> 风险 -> 任务 -> 交付 -> Codex 会话 -> Codex 交接的工作流路径、Codex 继续、本地检查结果、Finder、IDE、Terminal 和工作区链接复制放到同一个固定入口；状态标签采用中文优先，并把快捷动作收敛为交接、下一步和本地工具三组。
