...
- 支持本地自动化检查，可从 Rust Core、Swift/Rust 桥接和原生菜单栏生成刷新、风险、交付、任务、worktree、未提交服务信号。
...
原生菜单栏可以运行本地自动化检查。该检查会扫描工作区 Markdown 和 git 状态，生成刷新、风险、交付、任务、worktree、未提交服务信号，并在 Rust Core 桥接可用时写入 `automation.check.completed` 审计事件。
