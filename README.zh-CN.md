...
- 原生工作区详情顶部提供 Command Center，把生命周期进度、主路径推荐、范围 -> worktree -> 风险 -> 任务 -> 交付 -> Codex 交接的会话路径、Codex 继续、本地检查、Finder 和 Terminal 放到同一个固定入口。
...
每个工作区详情顶部都有 `Command Center`。它会汇总生命周期进度、分支确认状态、服务和 worktree 情况、风险等级、开放任务，并先给出一个带原因的主路径推荐；随后用会话路径把范围、worktree、风险、任务、交付和 Codex 交接压缩成一组可点击状态，再保留本地检查、Finder 和 Terminal 等工具入口。
