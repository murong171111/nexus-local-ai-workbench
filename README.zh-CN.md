...
每个工作区详情顶部都有 `Command Center`。它会汇总生命周期进度、分支确认状态、服务和 worktree 情况、风险等级、开放任务，并先给出一个带原因的主路径推荐；随后用会话路径把范围、worktree、风险、任务、交付和 Codex 交接压缩成一组可点击状态，再保留本地检查、Finder 和 Terminal 等工具入口。运行本地检查后，这里会保留一张检查回执，展示状态、风险/交付/任务/worktree 指标、审计写入状态和可复制摘要。
