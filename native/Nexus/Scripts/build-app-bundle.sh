...
usage() {
  cat <<'USAGE'
Usage: build-app-bundle.sh [--configuration release|debug] [--arch arm64] [--arch x86_64] [--output path/to/Nexus.app] [--disable-sandbox]

Builds the SwiftPM NexusNative executable and wraps it in a local macOS Nexus.app bundle.
The bundle is unsigned by default; signing, notarization, WidgetKit embedding, and DMG packaging remain separate M3 steps.
USAGE
}
