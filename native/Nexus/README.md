...
- Native Workflow action tray that groups document opens, local checks, and Agent handoffs by intent.
...
- Native Workflow validation/PR Codex handoff that copies local checks, delivery, SQL, tasks, services/worktrees, and PR summary requirements before opening Codex.
...
- Recent agent event sidebar feed backed by the Swift/Rust agent event bridge, with detail inspection, safe metadata actions, shared task drafts, confirmed `tasks.md` writeback, source-line task locators, Codex handoff prompts, copy-and-open Codex handoff, and JSON copy support.
...
- Native SQL readiness blocks delivery when `交付记录.md` declares SQL changes anywhere in the document, including SQL-section metadata such as `变更类型：DDL/DML`, affected tables, new fields, or backfill scripts, but `sql/` lacks either formal SQL or rollback SQL files.
...
- Native workspace detail includes a Workflow summary for open tasks, blocked tasks, delivery status, grouped document/check/Agent actions, delivery-readiness checks, inline local-check receipts, Codex handoff, and validation/PR handoff.
