...
- Semi-automated worktree setup: Nexus generates reviewable shell commands, explains missing worktrees, target branch availability in source repositories, source repository availability, and setup script visibility, and can create missing `repos/<service>` worktrees only after explicit confirmation.
