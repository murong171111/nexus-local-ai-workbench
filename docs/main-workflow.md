...
- task status writeback confirmation uses Swift Native local core to edit root `tasks.md`, append `workspace_task.updated` audit, show post-write checks for the edited row, focus the next active task, and refresh local checks when a task is closed
