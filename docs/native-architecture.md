...
- The native workspace detail surface now separates product workflow concerns: `Workflow` owns tasks and delivery state, `Risk Review` owns active risks and non-delivery readiness checks, and `Documents` owns standard Markdown/script entry points.
