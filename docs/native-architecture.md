...
- Create-workspace UX for scanning source repositories, filtering service candidates, accepting manual fallback services, showing pending scope, preflighting workspaces root readiness, folder validity, destination collisions, environment health, and scope warnings before confirmation, verifying generated files, initial status, and refresh-after-create scan visibility through an initialization receipt, and guiding the user to the next safe step after creation.
- Swift-owned workspace skeleton creation writes the standard Markdown set, SQL/log/repos/scripts directories, worktree command review script, `INDEX.md`, initialization receipt, and `workspace.created` audit event without depending on the Swift/Rust bridge.
...
- Target branch availability analysis.
- Risk detection.
...
- Legacy/reference standard workspace skeleton creation, kept for compatibility while Swift Native owns the Mac create-workspace write path.
...
- The native create-workspace sheet owns local-write preflight and the Swift Native write path for the setup step: it blocks write-failure conditions such as invalid folders or destination collisions, keeps branch/service uncertainty visible as review items for early demand scoping, then writes the workspace skeleton and receipt locally after explicit confirmation. After refresh, the receipt also records whether the new workspace is visible to the scanner; if the write succeeded but scanning does not return it, Nexus keeps the receipt visible as a warning instead of silently treating creation as complete. When no workspaces exist, it can prefill a demo workspace template, but the user still has to pass preflight and confirm before any files are written.
...
- Automation Action Center risk, delivery, task, branch, worktree, and dirty-service signals resolve a concrete target workspace before acting. Risk signals copy a risk-review handoff for the risky workspace; SQL delivery issues are preferred over generic delivery-record issues; task signals prefer high-priority or open-task workspaces; branch signals prefer workspaces with target-branch availability failures; worktree signals include missing services, source availability, script path, and branch-confirmation guidance. Copied automation prompts include the selected workspace's branch notes, delivery record, SQL artifacts, Nexus recommended next-step actions, and delivery/SQL check details when available.
...
- Confirmed task-status and lifecycle writebacks surface a local-write feedback card in the inspector, keeping affected-workspace focus, source-document review, and follow-up checks close to the write that changed local Markdown. Task-status updates now use a Swift Native store for root `tasks.md` edits and `workspace_task.updated` audit events, leaving the Rust bridge as compatibility/reference behavior.
