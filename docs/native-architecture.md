...
- Automation Action Center delivery and task signals resolve a concrete target workspace before acting. SQL delivery issues are preferred over generic delivery-record issues; task signals prefer high-priority or open-task workspaces; copied automation prompts include the selected workspace's delivery record, SQL artifacts, and delivery/SQL check details when available.
