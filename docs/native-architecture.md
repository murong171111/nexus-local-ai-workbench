...
- The native workspace list owns first-run and empty-filter guidance: it surfaces configured local paths, the latest environment-health summary, a team-profile -> environment-check -> workspace-create setup path, and the primary recovery actions before users reach a workspace detail.
