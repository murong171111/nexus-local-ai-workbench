...
- Automation Action Center delivery, task, and branch signals resolve a concrete target workspace before acting. SQL delivery issues are preferred over generic delivery-record issues; task signals prefer high-priority or open-task workspaces; branch signals prefer workspaces with branch-alignment failures. Copied automation prompts include the selected workspace's branch notes, delivery record, SQL artifacts, and delivery/SQL check details when available.
