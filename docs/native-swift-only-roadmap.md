...
- Replace Rust bridge dependencies for workspace scanning, document inventory, demand intake, readiness, git/worktree status, audit, settings, Codex sessions, search/index, and widget snapshot writing. `[started with NativeLocalCoreEvidence surfaced in Settings; AppState tracks 5/10 Swift-owned domains plus Git/Worktree, Audit, Widget Snapshot, and Search Index as partial Swift-owned domains; document preview and standard document recovery now use NativeDocumentStore before bridge fallback; demand intake status and initialization now use NativeDemandIntakeStore before bridge fallback; source repository scanning now uses NativeSourceRepositoryStore before bridge fallback; Codex session storage is Swift-owned, Native audit now writes JSONL and reads it back into workspace activity, and widget snapshot persistence plus NativeSearchIndexStore grouping/scope/fallback are Swift-owned while workspace scan/setup/index rebuild and bridge audit compatibility remain legacy-dependent]`
