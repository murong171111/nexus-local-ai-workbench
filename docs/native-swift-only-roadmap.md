...
- Replace Rust bridge dependencies for workspace scanning, document inventory, demand intake, readiness, git/worktree status, audit, settings, and widget snapshot writing. `[started with NativeLocalCoreEvidence surfaced in Settings; AppState tracks 4/8 Swift-owned domains plus Git/Worktree, Audit, and Widget Snapshot as partial Swift-owned domains; Native audit now writes JSONL and reads it back into workspace activity, and widget snapshot persistence is Swift-owned while scan/setup/snapshot generation and bridge audit compatibility remain legacy-dependent]`
