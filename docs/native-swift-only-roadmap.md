...
- Replace Rust bridge dependencies for workspace scanning, document inventory, demand intake, readiness, git/worktree status, audit, settings, Codex sessions, search/index, and widget snapshot writing. `[started with NativeLocalCoreEvidence surfaced in Settings; AppState tracks 8/10 Swift-owned domains plus workspace scanning and Git/Worktree as partial Swift-owned domains; document preview and standard document recovery now use NativeDocumentStore before bridge fallback; demand intake status and initialization now use NativeDemandIntakeStore before bridge fallback; workspace dashboards and service git/worktree rows now use NativeWorkspaceScanner before bridge fallback; source repository scanning now uses NativeSourceRepositoryStore before bridge fallback; missing worktree creation now uses NativeWorktreeSetupStore with Native audit JSONL; search index rebuild/search now use NativeSearchIndexStore; Widget snapshot generation/persistence now use NativeWidgetSnapshotBuilder and NativeWidgetSnapshotStore; Codex session storage and audit JSONL are Swift-owned, while remaining Git/worktree parity review is focused on UI recovery and deletion of legacy bridge contracts]`
