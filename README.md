...
- Native SwiftUI shell now has two primary surfaces: `Console` for focused execution and `Board` for a stage-based overview of filtered workspaces. Board columns follow the main requirement workflow, include All/Attention/Delivery/Archive scopes, summarize worktree attention, and selecting a card returns to Console with that workspace focused.
...
- Native service/branch confirmation evidence checks `services.md`, `branches.md`, workspace service rows, source repository availability, target branch availability, and branch policy before worktree setup; source repositories do not need to be checked out to the target branch when the branch/ref exists.
- Native worktree setup evidence checks missing workspace-local worktrees, target branch availability in source repositories, source repository availability, setup command visibility, and a service-level create/skip/blocked plan before development can continue; source current checkout is treated as context instead of a blocker.
- Native development-task evidence keeps root `tasks.md` as the execution source, chooses the next active task, blocks on unresolved task blockers, and shows a task plan that classifies work as resolve blocker, continue, queued, or closed with writeback guidance.
- Native delivery-gate evidence checks tasks, risks, service/worktree state, delivery records, SQL artifacts, dirty services, and local-check availability before marking delivery ready.
...
- Native SwiftUI Task Center that surfaces open workspace tasks from `tasks.md`, including task source-line locators, persisted filters, latest task writeback feedback, agent-sourced task writebacks with post-write focus/source actions, confirmed complete/defer actions, and task-level Codex copy-and-open handoff. Confirmed complete/defer writes update root `tasks.md` and `workspace_task.updated` audit events through Swift Native local core. Deferred tasks remain visible in the deferred filter without creating active automation or workflow blockers.
...
- Local automation checks for refresh, risk, delivery, target-branch availability, task, worktree, and dirty-service signals, built by Swift Native from current workspace summaries with the Rust Core bridge retained as a compatibility fallback, then exposed through the native menu bar, optional scheduled checks, visible local-check receipts, and configurable macOS notifications.
...
- Native create-workspace flow that scans source repositories, filters service candidates, selects real local services, leaves service scope pending when needed, offers a first-run demo template when no workspaces exist, checks root/folder/destination/environment/scope readiness before writing, then uses Swift Native to write the standard Markdown/SQL/log/repos/scripts skeleton, initialization receipt, `INDEX.md`, and `workspace.created` audit event before focusing the new workspace, opening `handoff.md`, and showing handoff, worktree, Codex, and check actions.
- Native worktree setup includes a Swift-owned evidence card and a confirmed preflight review for target branch readiness, missing worktrees, target branch availability in source repositories, source repositories, setup script visibility, and workspace-local write locations, then refreshes the workspace state after running and routes the next step to Finder, result-aware Codex handoff, or local checks.
...
- Native workspace Workflow now starts with a development-task evidence card for root `tasks.md`, showing active, blocked, completed, and deferred task evidence before delivery cleanup actions.
- Native workspace Workflow now also shows a delivery-gate evidence card before the detailed checklist, so delivery blockers, pending checks, review items, passed evidence, the ordered resolution plan, and the single next action are visible before scanning every row.
- Native workspace Workflow now includes validation/PR evidence between delivery and archive, summarizing local checks, delivery records, task/risk cleanup, PR/CI or release notes, and lifecycle readiness before the final archive decision.
...
- Native workspace Documents Hub for opening and previewing standard workspace files plus scanned `sql/*.sql` artifacts without leaving the detail view, including each document's purpose, update timing, gate participation, create policy, Markdown preview/source mode, explicit close-preview, retry, copy-path, Finder recovery, and confirmed creation of missing standard documents when a file is absent.
- Target branch availability checks that verify each service source repository has the specified target branch or a remote-tracking ref; a different currently checked-out worktree branch does not by itself block the workflow.
...
Archived workspaces remain visible in the workspace list and Archive filter, but they are excluded from active menu-bar counts, Task Center totals, and automation attention signals. Restoring one is explicit: the archive confirmation plan offers a restore-to-development lifecycle writeback, and the confirmation sheet explains that the workspace will re-enter active checks.
