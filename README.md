...
- Native SwiftUI Task Center that surfaces open workspace tasks from `tasks.md`, including task source-line locators, persisted filters, latest task writeback feedback, agent-sourced task writebacks, confirmed complete/defer actions, and task-level Codex copy-and-open handoff.
...
- Native workspace Command Center that puts lifecycle progress, a primary-path recommendation, task and delivery status summaries, a compact scope -> worktree -> risk -> task -> delivery -> Codex sessions -> handoff workflow path, Codex continuation, local-check results, Finder, IDE, Terminal, and workspace-link copy at the top of each detail view. Path cards show compact action labels, and the delivery card routes by status to local check, delivery handoff, validation/PR handoff, or document review, with Chinese-first status labels and quick actions grouped into handoff, next-step, and local tool lanes.
...
- Native workspace detail overview that keeps lifecycle, branch, services, risk, tasks, delivery, Codex session count, and latest local-check state visible before deeper workflow sections. Overview tiles are actionable and route directly to the matching document, worktree setup, risk handoff, delivery handoff, Codex session action, or local check.
...
- Native sidebar Agent Events now render as an Agent Inbox with Attention and Recent groups, so permission, question, tool-review, and error events appear before informational events; when there are no events, the sidebar shows a clear empty state. Agent Event detail actions can copy a Codex continuation pack or copy it and open Codex in one step, with local audit records for both paths. Permission, question, and tool-review events also get an Agent action surface with copyable approve/deny/answer/review response templates that do not execute command metadata.
...
- Native Workflow delivery-readiness checklist rows route directly to the nearest action for branch, services/worktrees, tasks, risks, delivery records, SQL checks, and dirty services, keeping delivery cleanup inside the Workflow section.
...
Recent Agent Events in the native sidebar are organized as an Agent Inbox. Attention shows permission, question, tool-review, and error events first; Recent keeps the remaining informational events nearby, and an empty state confirms there is nothing waiting for review. Opening an event shows detail review where Nexus can select the matching workspace, open safe local paths or web links, copy the shared Codex continuation pack, or copy that pack and open the configured Codex URL in one action. Permission, question, and tool-review events show an Agent action surface with copyable approve/deny/answer/review templates; these copies are audited, but command metadata remains review-only text and is not executed by Nexus.
