...
- Native worktree setup evidence checks missing workspace-local worktrees, branch mismatches, source repository availability, setup command visibility, and a service-level create/skip/blocked plan before development can continue.
...
- Native worktree setup includes a Swift-owned evidence card and a confirmed preflight review for target branch readiness, missing worktrees, branch alignment, source repositories, setup script visibility, and workspace-local write locations, then refreshes the workspace state after running and routes the next step to Finder, result-aware Codex handoff, or local checks.
