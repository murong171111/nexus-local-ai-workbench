...
- Native Agent Event detail actions for copying a Codex continuation pack or copying it and opening Codex in one step, with local audit records for both paths. Permission, question, and tool-review events also get an Agent action surface with copyable approve/deny/answer/review response templates that do not execute command metadata.
