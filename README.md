...
Recent Agent Events in the native sidebar can be opened for detail review. From that sheet, Nexus can select the matching workspace, open safe local paths or web links, copy the shared Codex continuation pack, or copy that pack and open the configured Codex URL in one action. Event handoffs are audited as local operational context; command metadata remains review-only text and is not executed by Nexus.
