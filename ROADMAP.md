...
- Add native demand-intake preflight for workspace-local requirement confirmation before development. `[started with Swift/Rust bridge status refresh, confirmed no-overwrite initialization, per-file readiness, document open actions, local write feedback, and $lanhu-demand-intake Codex prompt copy/open handoff]`
...
- Add a session startup flow that converts readiness results into prioritized Codex/worktree/document actions. `[started in Rust Core, Tauri preview UI, and native SwiftUI shell with active-task health signals plus a Command Center-adjacent next-step queue]`
