...
- Add a workspace detail Command Center so the native shell has one primary path before deeper Workflow, Risk Review, Documents, and Activity sections. `[started in native SwiftUI shell with detail status overview, primary-path guidance, a compact workflow path for scope, worktree, risk, tasks, delivery, Codex sessions, and handoff, Chinese-first status labels, grouped Handoff/Next/Local quick actions, workspace link copy, plus local-check receipts]`
