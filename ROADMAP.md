...
- Add a workspace detail Command Center so the native shell has one primary path before deeper Workflow, Risk Review, Documents, and Activity sections. `[started in native SwiftUI shell with primary-path guidance and a compact session path for scope, worktree, risk, tasks, delivery, and handoff]`
