...
- Add a workspace detail Command Center so the native shell has one primary path before deeper Workflow, Risk Review, Documents, and Activity sections. `[started in native SwiftUI shell with detail status overview, primary-path guidance, a compact session path for scope, worktree, risk, tasks, delivery, and handoff, plus local-check receipts]`
