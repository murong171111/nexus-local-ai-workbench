...
- Add unit coverage for workspace parsing, creation defaults, widget snapshots, and git status mapping. `[started with JS coverage for workspace parsing, creation defaults, widget snapshots, search grouping, settings profiles, browser-preview sample quality, agent hooks, and Rust Core git/workspace tests]`
