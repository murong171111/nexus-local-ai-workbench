...
- Add a native workflow summary for task and delivery status in the workspace detail. `[started in native SwiftUI shell with delivery focus guidance, delivery-update Codex handoff, task source-line locators, task writeback feedback, delivery-readiness checks, inline local-check receipts, full-document SQL artifact enforcement for formal/rollback files, and lifecycle writeback recommendations]`
